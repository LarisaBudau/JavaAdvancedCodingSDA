package com.sda.larisabudau.advanced.coading.oop.ex1;

import java.util.function.Predicate;

/**Implement the Validator interface, which will include a boolean validate(Parcel input)
 * method in its declaration. Create a Parcel class with the parameters:
 * • int xLength
 * • int yLength
 * • int zLength
 * • float weight
 * • boolean isExpress
 * The validator should verify that the sum of the dimensions (x, y, z) does not exceed 300, whether each size is not
 * less than 30, whether the weight does not exceed 30.0 for isExpress = false or 15.0 for isExpress = true.
...
public class ParcelValidator implements Predicate<Parcel> {
    ...
    public boolean test(Parcel parcel) {
        ...
        if (parcel.isExpress() && parcel.getWeight() > 15) {
            System.out.println("he weight of you express package is greater than 15");
            return false;
        }

        if (parcel.isExpress() && parcel.getWeight() > 30) {
            System.out.println("he weight of you express package is greater than 30");
            return false;
        }
        return true;
    }
